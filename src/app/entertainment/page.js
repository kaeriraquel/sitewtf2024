...
import entertainmentList from "../components/games.json";

export default function entertainment() {
  const productList = entertainmentList.gamesList.map((product) => (
    <div key={product.id}>
      {/* Check this so that the image fills up the correct way */}
      <Link href={"Entertainment/" + product.id}>
        <div className="rounded-lg h-12 w-32 md:h-32 md:w-64 bg-wtfGrey place-content-center">
          {/* <img
              ...
              src={product.logo}
            /> */}
          <div className="sf-font text-center text-wtfYellow text-md md:text-xl md:p-4 font-bold justify-self-center">
            {product.title}
          </div>
        </div>
      </Link>
